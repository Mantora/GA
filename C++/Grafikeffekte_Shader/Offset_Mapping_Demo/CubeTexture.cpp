...
{
	int ok=0;
	if( pd->CreateCubeTexture( size, 0, D3DUSAGE_RENDERTARGET, D3DFMT_X8R8G8B8, D3DPOOL_DEFAULT, &pt )
		== D3D_OK )ok=1;
	
	if(!ok)
	{
		if( pd->CreateCubeTexture( size, 0, D3DUSAGE_RENDERTARGET, D3DFMT_R8G8B8, D3DPOOL_DEFAULT, &pt )
		== D3D_OK )ok=1;
	}
...
static inline void PackVectorInByte(const vec &v, unsigned char* data)
{
	data[0] = (unsigned char) ( (v.x+1.0f)*127.5f );	//((v.x+1.0f)/2.0f*255.0f);
	data[1] = (unsigned char) ( (v.y+1.0f)*127.5f );
	data[2] = (unsigned char) ( (v.z+1.0f)*127.5f );
}

int CubeTexture::MakeCubeMapNormalizer(int size, int filter)
{
	// right, left, top, bottom, back, front
	Image i[6];
	int x,y,z;
	...
	for(int j=0; j<6; j++)
	{
		...
		i[j].sizeX = size;
		i[j].sizeY = size;
		i[j].data = new unsigned char[size*size*3];
		if(i[j].data==NULL)return 0;			// delete ???
	}
	float halfsize = (float)size*0.5f;
	
	for( y=0; y<size; y++)		// GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB
	{
		for( z=0; z<size; z++)
		{
			v.set( halfsize, halfsize - (float)y, halfsize - (float)z);
			v.Normalize();
			PackVectorInByte( v, &i[0].data[(y*size+z)*3]);
		}
	}

	for( y=0; y<size; y++)		// GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB
	{
		for( z=0; z<size; z++)
		{
			v.set( -halfsize, halfsize - (float)y, (float)z - halfsize);
			v.Normalize();
			PackVectorInByte( v, &i[1].data[(y*size+z)*3]);
		}
	}

	for( z=0; z<size; z++)		// GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB
	{
		for( x=0; x<size; x++)
		{
			v.set( (float)x - halfsize, halfsize, (float)z - halfsize);
			v.Normalize();
			PackVectorInByte( v, &i[2].data[(z*size+x)*3]);
		}
	}

	for( z=0; z<size; z++)		// GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB
	{
		for( x=0; x<size; x++)
		{
			v.set( (float)x - halfsize, -halfsize, halfsize - (float)z);
			v.Normalize();
			PackVectorInByte( v, &i[3].data[(z*size+x)*3]);
		}
	}

	for( y=0; y<size; y++)		// GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB
	{
		for( x=0; x<size; x++)
		{
			v.set( (float)x - halfsize, halfsize - (float)y, halfsize);
			v.Normalize();
			PackVectorInByte( v, &i[4].data[(y*size+x)*3]);
		}
	}

	for( y=0; y<size; y++)		// GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB
	{
		for( x=0; x<size; x++)
		{
			v.set( halfsize - (float)x, halfsize - (float)y, -halfsize);
			v.Normalize();
			PackVectorInByte( v, &i[5].data[(y*size+x)*3]);
		}
	}
...
//	FILE* file;	file = fopen( "pokus.dds","wb");fwrite( data, 1, sizeof(z_dds)+6*i[0].sizeX*i[0].sizeY*i[0].planes, file);	fclose(file);
		...
		!= D3D_OK )
	{
		pt = NULL; 
		if(data!=NULL)delete [] data;
		MessageBox( hWnd, "Problem pri nahravani cube mapy pre normalizaciu", "ERROR", MB_OK );
		return 0;
	}
	if(data!=NULL)delete [] data;
	return 1;
