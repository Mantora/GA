...
inline int PointInBox(const vec &a, const vec &min, const vec &max)
{
	if(a.x<min.x || a.x>max.x) return 0;
	if(a.y<min.y || a.y>max.y) return 0;
	if(a.z<min.z || a.z>max.z) return 0;
	return 1;
}

inline int SphereInBox( const vec &center, float radius, vec min, vec max)
{
	min -= radius;
	max += radius;
	if(PointInBox(center, min, max))return 1;
	return 0;
}

inline void ModifyBox(vec &min, vec &max, const vec &v)
{
...
public:
	...
	inline vec4( const vec &in, const float d = 1.f);
...
inline vec4::vec4( const vec &in, const float d)
{	x = in.x; y = in.y; z = in.z; w = d;}
