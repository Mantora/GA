...
{
	if(LoadNoMessage(filename)) return 1;
	MessageBox( hWnd, filename, "Nenájdený súbor", MB_OK );
	return 0;
}

int Image::LoadNoMessage(char *filename)
{
	char p[5];
	...
	if( !strcmp(p,"bmp"))		return LoadBMP( filename);
	else if( !strcmp(p,"jpg"))	return LoadJPG( filename);
	else if( !strcmp(p,"jpeg"))	return LoadJPG( filename);
	else if( !strcmp(p,"tga"))	return LoadTGA( filename);
	return 0;
}
...
int Image::SaveTGA(char *filename)
{
	FILE *sub;
	unsigned char ctmp = 0, type, mode;
	short int width = sizeX, height = sizeY, itmp = 0;
	unsigned char pixelDepth = 8*planes;
	...
	fwrite(data, planes*sizeX*sizeY, 1, sub);
	fclose(sub);
	return 1;
}
...
unsigned char* Image::Get(int *size)
{
	...
	int line_size = planes*sizeX;
	if(line_size%4)line_size += 4 - line_size%4;
	if(data==NULL || planes!=3){ *size=0; return 0;}
	...
	infoheader.biHeight = sizeY;	// Height of image			| vyska obrazka - sizeY
	infoheader.biPlanes = 1;		// Number of colour planes	| pocet farebnych rovin musi bit 1
	infoheader.biBitCount = planes*8;	// Bits per pixel			| bitov na pixel moze bit 1,4,8,24
	infoheader.biCompression = 0;	// Compression type			| typ compresie , 0 - bez kompresie
	infoheader.biSizeImage = line_size*infoheader.biHeight ;	// Image size in bytes		| velkost obrazka v byte
	infoheader.biXPelsPerMeter = 0;	// Pixels per meter X		| pixelov na meter v smere x
	infoheader.biYPelsPerMeter = 0;	// Pixels per meter Y		| pixelov na meter v smere y
...
int Image::Make(int size_x, int size_y, int planes)
{
	if(planes<0 || planes>4)return 0;
	if(size_x<1 || size_y<1) return 0;
	if(data!=NULL)delete [] data;
	Image::planes = planes;
	sizeX = size_x; 
	sizeY = size_y; 
...
float pow_fast(float x, int exp)
{
	if(exp>1)
	{
		if(exp % 2)
		{
			float a = pow_fast(x,exp/2);
			return x*a*a;
		}
		else
		{
...
int Image::MakeExponentTable1D(int size_x, float exp)
{
	if(!Make(size_x,1,3))return 0;
	for(int x=0; x<size_x; x++)
	{
	//	float out = pow_fast( x/(float)(size_x-1), (int)exp);
		float out = (float)pow( x/(float)(size_x-1), exp);
...
int Image::MakeExponentTable2D(int size_x, int size_y, float min_exp)
{
	if(!Make(size_x,size_y,3))return 0;
	for(int y=0; y<size_y; y++)
	{
		for(int x=0; x<size_x; x++)
		{
		//	float out = pow_fast( x/(float)(size_x-1), (int)min_exp+(size_y-1-y));
			float out = (float)pow( x/(float)(size_x-1), min_exp+(size_y-1-y));
			unsigned char ch = (unsigned char)(out*255.f);
			data[(y*sizeX+x)*planes+0] = ch;
			data[(y*sizeX+x)*planes+1] = ch;
			data[(y*sizeX+x)*planes+2] = ch;
		}
	}
	return 1;
}
...
int Image::MakeExponentTable2D_NL_NH(int size_x, int size_y, float exp, int channels)
{
	if(channels==2)
	{
		if(!Make(size_x,size_y,2))return 0;
		for(int y=0; y<size_y; y++)
		{
		//	float NH = pow_fast( (size_y-1-y)/(float)(size_y-1), (int)exp);
			float NH = (float)pow( (size_y-1-y)/(float)(size_y-1), exp);
			for(int x=0; x<size_x; x++)
			{
				float NL = (float)x/(float)(size_x-1);
				
				data[(y*sizeX+x)*planes+0] = (unsigned char)(NL*255.f);
				data[(y*sizeX+x)*planes+1] = (unsigned char)(NH*255.f);
			}
		}
		return 1;
	}
	...
	{
		if(!Make(size_x,size_y,4))return 0;
		for(int y=0; y<size_y; y++)
		{
		//	float NH = pow_fast( (size_y-1-y)/(float)(size_y-1), (int)exp);
			float NH = (float)pow( (size_y-1-y)/(float)(size_y-1), exp);
			for(int x=0; x<size_x; x++)
			{
				...
				data[(y*sizeX+x)*planes+0] = (unsigned char)(NL*255.f);
				data[(y*sizeX+x)*planes+1] = (unsigned char)(NL*255.f);
				data[(y*sizeX+x)*planes+2] = (unsigned char)(NL*255.f);
				data[(y*sizeX+x)*planes+3] = (unsigned char)(NH*255.f);
			}
		}
		return 1;
	}
...
unsigned char* Image::Get_dds(int *size)
{
	unsigned char* pdata;
	unsigned char* p;
	...
	pdata = new unsigned char[sizeof(z_dds)+line_size*sizeY];
	*size = sizeof(z_dds)+line_size*sizeY;
	z_dds *dds;
	dds = (z_dds*)pdata;
	...
	dds->dwHeight = sizeY;
	dds->dwWidth = sizeX;
	dds->dwPitchOrLinearSize = line_size;
	dds->dwDepth = 0;
	dds->dwMipMapCount = 0;

	dds->ddpfPixelFormat.dwSize = sizeof(z_dds_DDPIXELFORMAT);
	if(planes==4)dds->ddpfPixelFormat.dwFlags = DDPF_RGB | DDPF_ALPHAPIXELS;
	else dds->ddpfPixelFormat.dwFlags = DDPF_RGB;
	dds->ddpfPixelFormat.dwRGBBitCount = 8*planes;
	...
	if(planes==4)dds->ddpfPixelFormat.dwRGBAlphaBitMask = 0xff000000;
	dds->ddsCaps.dwCaps1 = DDSCAPS_TEXTURE;
	dds->ddsCaps.dwCaps2 = 0;
	
	p = &pdata[sizeof(z_dds)];
	for(int y=0; y<sizeY; y++)
	{
		for(int x=0; x<sizeX; x++)
		{
			if(planes>2)
			{
				if(isBGR)
				{
					p[y*line_size+x*planes+0] = data[((sizeY-y-1)*sizeX+x)*planes+0];
					p[y*line_size+x*planes+1] = data[((sizeY-y-1)*sizeX+x)*planes+1];
					p[y*line_size+x*planes+2] = data[((sizeY-y-1)*sizeX+x)*planes+2];
					if(planes==4)
						p[y*line_size+x*planes+3] = data[((sizeY-y-1)*sizeX+x)*planes+3];
				}
				else
				{
					p[y*line_size+x*planes+0] = data[((sizeY-y-1)*sizeX+x)*planes+2];
					p[y*line_size+x*planes+1] = data[((sizeY-y-1)*sizeX+x)*planes+1];
					p[y*line_size+x*planes+2] = data[((sizeY-y-1)*sizeX+x)*planes+0];
					if(planes==4)
						p[y*line_size+x*planes+3] = data[((sizeY-y-1)*sizeX+x)*planes+3];
				}
			}
			else if(planes==2)
			{
				...
				p[y*line_size+x+0] = data[((sizeY-y-1)*sizeX+x)+0];
		}
	}

//	FILE* file;	file = fopen( "pokus.dds","wb");fwrite( data, 1, sizeof(z_dds)+sizeX*sizeY*planes, file);	fclose(file);
...
int Image::MakeAttenuationTable1D(int size_x, int equation)
{
	if(!Make(size_x,1,3))return 0;
	for(int x=0; x<size_x; x++)
	{
		float out;
		float d2 = x/(float)(size_x-1);
		switch(equation)
		{
		case 0: out = 1.f - d2; break;
		case 1: out = 1.f - d2; out*=out; break;
		case 2: out = (float)sqrt(1.f - d2); break;
...
int Image::setPlanes(int count, int copy_from_plane)
{
	unsigned char *old_data = data;
	unsigned int old_planes = planes;

	if(count<=planes)return 0;
	if(count<1 || count>4)return 0;
	if(data==NULL)return 0;
	if(copy_from_plane>=planes || copy_from_plane<0)return 0;
	...
	for(int y=0; y<sizeY; y++)
	{
		for(int x=0; x<sizeX; x++)
		{
			            data[(y*sizeX+x)*planes+0] = old_data[(y*sizeX+x)*old_planes+copy_from_plane];
			if(planes>1)data[(y*sizeX+x)*planes+1] = old_data[(y*sizeX+x)*old_planes+copy_from_plane];
			if(planes>2)data[(y*sizeX+x)*planes+2] = old_data[(y*sizeX+x)*old_planes+copy_from_plane];
			if(planes>3)data[(y*sizeX+x)*planes+3] = old_data[(y*sizeX+x)*old_planes+copy_from_plane];
		}
	}
	delete [] old_data;
	return 1;
...
float Image::getPointBilinear(float x, float y, int plane)
{
	double i=x,j=y,zx,zz;    // zx,zz zvysok
	int x0,x1,y0,y1;
	int x0_,y0_;
	...
	if(x0_ >= sizeX-1)x0_=sizeX-2;
	if(y0_ >= sizeY-1)y0_=sizeY-2;

	zx = fmod(i,1.0);
	zz = fmod(j,1.0);

	x0 = getPoint(x0_,y0_,plane);
	x1 = getPoint(x0_+1,y0_,plane);
	y0 = getPoint(x0_,y0_+1,plane);
	y1 = getPoint(x0_+1,y0_+1,plane);

	i = zx*(float)(x1-x0) + (float)x0; 
	j = zx*(float)(y1-y0) + (float)y0;
	return (float)(zz*(j-i)+i)/255.f;
}
