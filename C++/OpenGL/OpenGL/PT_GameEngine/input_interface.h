		...
		long check( void )
		{
			...
			if( PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) )
			{
				switch( msg.message )
				{
					case WM_KEYDOWN:
					{
						event_key = msg.wParam;
					}
					break;

					case WM_MOUSEMOVE:
					{
						mouse_act_pos.wx = short( msg.lParam & 0xffff );
					...
					case WM_LBUTTONDOWN:
					{
						mouse_left_state = -1;  mouse_left_downpos = mouse_act_pos;
					}
					break;

					case WM_LBUTTONUP:
					{
						mouse_left_state = 1;   mouse_left_uppos = mouse_act_pos;
					}
					break;
